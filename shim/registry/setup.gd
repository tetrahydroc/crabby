...
##
## A "plan" is an Array of [verb, ...args] entries. Order is insertion
## order, so register-then-patch flows work naturally. Each entry maps
## to one of the existing public verbs (register, override, patch,
## append, prepend, remove_from, revert, remove, hooks) or one of the
## aggregators (register_item / register_weapon / register_magazine /
## register_attachment / register_furniture / register_ai_loadout),
## plus a meta verb `when` for conditional sub-plans.
##
## Why this exists: mods that lean heavily on the hook + registry
## systems tend to accumulate dozens of administrative lines in _ready
## (one per hook, one per registration). setup() collapses that into
## one declarative literal that can be a const at module scope or a
## runtime-built local in _ready.
##
...
## const plan with non-Callable predicates evaluates them at script-
## parse time -- fine for compile-time bools but wrong for runtime
## state, so prefer Callable / lambda predicates in const plans.
##
...
##
## Failures isolate. A bad entry produces a result with ok=false and
## the next entry runs anyway -- consistent with the singular verbs and
## the _many forms.
...
# When-shape: [when, predicate, sub_plan]. Recurse on sub_plan if
# predicate evaluates truthy. Skipped when-blocks return ok=true
# (vacuously: nothing failed because nothing ran).
func _setup_dispatch_when(arr: Array) -> Dictionary:
	...
	# Predicate true -- recurse. Inner plan returns its own
	# {ok, results}; bubble up the same shape so callers can introspect
	# nested outcomes.
	var inner: Dictionary = setup(arr[2])
	return {"verb": "when", "evaluated": true, "ok": inner.ok, "results": inner.results}


# Predicate accepts plain bool, Callable, or anything bool()-coercible.
# Any other type is treated as false (with a warning) -- safer than
# running the sub-plan on a typo.
...
# These helpers wrap the _many methods as Callables so the dispatcher
# can pass them through _setup_dispatch_many. Saves repetition.
...
# Aggregator-shape: [verb, {id: data, ...}]. No `reg` arg -- each
# aggregator is registry-implicit. Maps to the public aggregator
# helpers, which already accept and return a Dictionary.
